...
export const DISCOVERY_CONFIG = {
  ...
  // 카테고리별 발견 반경(m). 음식·카페는 '걸어갈 거리'로 좁혀 현재 위치에 민감하게
  // (1km 이동 시 목록이 확 바뀜). 관광·쇼핑·문화·자연은 '이동해서 가는 곳'이라 넓게.
  radii: {
    추천: [800, 2000],
    음식점: [500, 1100],
    카페: [400, 1000],
  ...
  radiiDefault: [800, 2000],
  minRating: 4.0,
  // 카테고리 밀도에 맞춘 리뷰 임계값. 치앙마이는 KL 보다 도시 규모가 작아 같은 급의
  // 맛집·상점도 리뷰수가 절반 수준 → 음식/쇼핑 바를 낮추지 않으면 목록이 비어 보인다.
  minReviews: {
    추천: 600, // 메인 탭: 주변 '화제 장소'만(높은 바)
    음식점: 600,
    카페: 300,
    관광명소: 500,
    쇼핑: 600,
...
// ---- 카테고리 해석 (Google primaryType → 단일 탭) ----
// primaryType 만으로 판정한다(types 폴백 없음). types 를 보면 매장 안에 식당/카페가 있는
// 호텔·랜드마크·문화공간이 음식점/카페로 새어 들어오므로, "그 장소가 본질적으로 무엇인가"를
// 나타내는 primaryType 을 권위로 삼는다. _restaurant 접미사로 요리별 변형을 한 번에 흡수.
const TYPE_TO_CAT = new Map();
for (const [cat, list] of [
  ["음식점", [
    "restaurant", "food_court", "meal_takeaway", "meal_delivery", "food", "noodle_shop",
    "fast_food_restaurant", "diner",
  ]],
  ["카페", [
    "cafe", "coffee_shop", "coffee_roastery", "bakery", "bagel_shop", "donut_shop",
    "tea_house", "dessert_shop", "dessert_restaurant", "ice_cream_shop", "juice_shop",
    "cafeteria", "confectionery",
  ]],
  ["쇼핑", [
  ...
  ["자연·공원", [
    ...
    "natural_feature", "lake", "waterfall", "wildlife_park", "wildlife_refuge", "zoo", "aquarium",
  ]],
  ["문화·역사", [
  ...
  for (const t of list) if (!TYPE_TO_CAT.has(t)) TYPE_TO_CAT.set(t, cat);
}

// override → primaryType 권위(접미사 → 매핑) → 기본 관광명소.
// (types 인자는 하위호환을 위해 받지만 더 이상 사용하지 않는다.)
export function categoryOf(primaryType, types = [], override) {
  if (override) return override;
  if (primaryType) {
    if (/_restaurant$/.test(primaryType)) return "음식점"; // *_restaurant 전부 흡수
    if (TYPE_TO_CAT.has(primaryType)) return TYPE_TO_CAT.get(primaryType);
  }
  return "관광명소";
}

// 탭→Google Nearby includedTypes. 이 맵에 있는 카테고리만 자동발견한다.
// "추천"(전체 탭)은 여행자 관점의 혼합 타입으로 출발지 주변 인기 장소를 한 번에 발견한다.
// worker/src/index.js 의 NEARBY_INCLUDED_TYPES 와 동기화할 것. 모든 타입은 Nearby 유효성 검증됨.
export const INCLUDED_TYPES = {
  추천: ["restaurant", "cafe", "tourist_attraction", "shopping_mall"],
  ...
  // 치앙마이는 사원(buddhist_temple)이 문화 카테고리의 사실상 전부라 반드시 포함한다.
