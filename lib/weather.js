...
export async function updateWeather(origin) {
  ...
  try {
    ...
    // Open-Meteo 실패 시: 컨테이너는 건드리지 않고 날씨 행만 숨긴다.
    // (data.gov.my 예보 줄이 살아 있으면 칩은 그 줄만으로 계속 표시됨)
...
// 경보 제목(영문)에서 키워드로 한글·아이콘 추론. 정확 일치가 아니라 부분일치라
// "First/Second Category Warning on ...", "Continuous Rain Warning" 등 변형·접두에도 견고.
function warnMeta(title) {
  const t = String(title || "");
  if (/thunderstorm|ribut petir/i.test(t)) return { ko: "천둥번개·호우 경보", icon: "⛈️" };
  if (/continuous rain|heavy rain|hujan/i.test(t)) return { ko: "호우 경보", icon: "🌧️" };
  if (/strong wind|rough seas|angin/i.test(t)) return { ko: "강풍·풍랑 경보", icon: "🌊" };
  if (/earthquake|tsunami|gempa/i.test(t)) return { ko: "지진·해일 경보", icon: "🌐" };
  return { ko: t, icon: "⚠️" }; // 폴백: 원문 + 일반 경고
}
...
export async function updateForecast(workerUrl) {
  ...
  try {
    ...
    mainLine.textContent = `오늘 KL ${when}${desc}`.trim();
    ...
    if (f.minTemp != null && f.maxTemp != null) {
      ...
      tempLine.textContent = `${f.minTemp}–${f.maxTemp}°`;
      node.appendChild(tempLine);
    }
    node.hidden = false;
    // 날씨 fetch 와 독립: 예보만 성공해도 칩 컨테이너를 노출한다.
    const widget = el("heroWeather");
